...
- `references/localization.md` — where user-facing text lives (`composeResources/values*/strings.xml`), the English-default/`values-es` split, and the rule that a ViewModel exposes an error *type* a `Labels.kt` translates, never a string.
...
- A column never repeats what another table already determines: `menu_items` carries only price, position and availability for its `(menu_id, dish_id)` key; currency belongs to the restaurant, and a value derivable from the row (the old `dishes.allergen_source`) is derived, not stored.
- A genuine many-to-many gets its own table (`menu_items`, and a review's target via `restaurant_reviews`/`dish_reviews`), but a plain one-to-many stays a column: `dishes.restaurant_id` is a functional dependency on the dish key, and splitting it into a bridge table only bought a join. The domain models (`Dish.restaurantId`, `ReviewTarget`) never change shape because of how storage is laid out.
- An RPC that inserts a row whose readability depends on a *second* row must not read it back with `returning` — `returning` evaluates the SELECT policy too.
...
- Models live in a `model/` package inside their own layer and module — `…/<feature>/domain/model/`, `…/<feature>/ui/model/`, `org.shareat.app.domain.model` for the shared base, `org.shareat.app.data.supabase.model` for DTOs — never declared alongside the use case, repository, or ViewModel that consumes them. Supabase domain mappers live in `org.shareat.app.data.supabase.mapper`, grouped per aggregate.
- One repository per file: each repository interface goes in a file named after it (`MenuRepository.kt`), and so does each implementation (`SupabaseMenuRepository.kt`, `FakeMenuRepository.kt`).
...
- The app draws edge to edge, so a screen applies the top window inset exactly once: `Modifier.safeDrawingTopPadding()` from `:shared:designsystem` on its root `Column`, or the `Scaffold`/`TopAppBar` that already handles it — never both, never a hardcoded `dp` standing in for the status bar.
- No user-facing literal in a composable, ViewModel, mapper or ui state: text lives in the module's `composeResources/values/strings.xml` (English, the fallback) and `values-es/strings.xml` (Spanish), and a ViewModel exposes an error enum that a `Labels.kt` next to the composables resolves with `stringResource` — see `references/localization.md`.
