...
- Relationships that are not attributes live in their own table: a dish's restaurant is `restaurant_dishes`, and a review's target is `restaurant_reviews`/`dish_reviews` under the shared `reviews` parent. The domain models (`Dish.restaurantId`, `ReviewTarget`) do not change shape because storage split a table.
- An RPC that inserts a row whose readability depends on a second row must not read it back with `returning` — `returning` evaluates the SELECT policy too. Generate the id instead (see `save_restaurant_dish`).
