...
/** Android `actual` counterpart of the shared `expect` processor, using FileKit compression. */
actual class FileKitDishImageProcessor actual constructor() : DishImageProcessor {
    override suspend fun invoke(displayName: String, bytes: ByteArray): Result<ProcessedDishImage> = process(displayName, bytes)
}

private suspend fun process(displayName: String, bytes: ByteArray): Result<ProcessedDishImage> = try {
